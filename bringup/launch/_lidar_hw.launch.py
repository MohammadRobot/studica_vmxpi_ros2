...
"""Private YDLIDAR launch with optional static TF configuration."""
...
def _launch_setup(context, *args, **kwargs):
    ...
    lidar_parent_frame = LaunchConfiguration("lidar_parent_frame").perform(context).strip()
    lidar_child_frame = LaunchConfiguration("lidar_child_frame").perform(context).strip()
    ...
    driver_node = LifecycleNode(
        ...
        output="screen",
        emulate_tty=True,
        parameters=[ydlidar_params_file, {"frame_id": lidar_child_frame}],
        namespace="/",
    )
    ...
    if lidar_child_frame == "laser_scan_frame":
        actions.append(
            LogInfo(
                msg=(
                    "LiDAR uses laser_scan_frame from robot_description; "
                    "no duplicate static TF will be published."
                )
            )
        )
    else:
...
def generate_launch_description():
    declared_arguments = [
        ...
        DeclareLaunchArgument(
            "lidar_child_frame",
            default_value="laser_scan_frame",
